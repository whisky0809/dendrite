...
export function registerDendriteCli(ctx: {
  ...
  configCmd
    ...
    .action(async () => {
      let editor = process.env.EDITOR || process.env.VISUAL || "vi";

      if (!isSafeEditor(editor)) {
        console.warn(`Warning: EDITOR/VISUAL environment variable "${editor}" is not in the safe whitelist. Falling back to "vi".`);
        editor = "vi";
      }
